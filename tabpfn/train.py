        ...
        for batch, (data, targets, single_eval_pos) in pbar:
            ...
            with cm:
                ...
                with autocast(enabled=scaler is not None):
                    ...
                    output = model(tuple(e.to(gpu, non_blocking=True) if torch.is_tensor(e) else e for e in data) if (isinstance(data, tuple) or isinstance(data, list)) else data.to(device)
                    ...
                    if isinstance(criterion, nn.GaussianNLLLoss):
                        ...
                        losses = criterion(mean_pred.flatten(), targets.to(gpu, non_blocking=True).flatten(), var=var_pred.flatten())
                    elif isinstance(criterion, (nn.MSELoss, nn.BCEWithLogitsLoss)):
                        losses = criterion(output.flatten(), targets.to(gpu, non_blocking=True).flatten())
                    elif isinstance(criterion, nn.CrossEntropyLoss):
                        losses = criterion(output.reshape(-1, n_out), targets.to(gpu, non_blocking=True).long().flatten())
                    else:
