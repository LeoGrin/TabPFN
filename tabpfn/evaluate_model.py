...
def get_validation_performance(model, metric="accuracy", suites=[337, 334],
    ...
    for k, suite_id in enumerate(suites):
        ...
        for task_id in accepted_tasks:
            # replace by concat
            df = pd.concat([df, pd.DataFrame({'task_id': task_id, 'model': 'model', 'score': results_model[task_id]}, index=[0])])
            for name, baseline in baselines:
                df = pd.concat([df, pd.DataFrame({'task_id': task_id, 'model': name, 'score': results_baselines[task_id][name]}, index=[0])])
